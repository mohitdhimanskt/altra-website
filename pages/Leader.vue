<template>
   <div data-server-rendered="true">
        <!---->
        <div id="layout">
            <div class="home">
                <div class="header-logo other-logo">
                    <!-- new header-->
                        <nav>
                           <div class="menu-icon">
                              <i class="fa fa-bars fa-2x"></i>
                           </div>
                           <div class="logo">
                         <nuxt-link to="/"><img src="~/assets/uploads/text-logo.png" class="text-logo img-responsive" alt="altra travels"/></nuxt-link>
                           </div>
                           <div class="menu">
                             <ul>
                                <li><nuxt-link to="treks">Treks</nuxt-link></li>
                                <li><nuxt-link to="leader">Trek Leads</nuxt-link></li>
                                <li><nuxt-link to="/">Blog</nuxt-link></li>
                               <li> <nuxt-link to="about">About Us</nuxt-link></li>
                                <li><nuxt-link to="contact">Contact Us</nuxt-link></li>
                                <li><nuxt-link to="sign"> Sign In</nuxt-link></li>
                              </ul>
                           </div>
                        </nav>

                    <!-- new header ends-->
                </div> <!-- header-logo ends-->


        <div class="leaders-page padding-top-1 altra-heading">
<div class="container">
    <h2 class="altra-row-title text-left primary-color divider-text"><span>Trek Leaders </span></h2>
    <h4 class="altra-row-text">Know Our Trek Leaders</h4>
     <div class="row">
                            <div class="col-12 col-md-4 col-lg-3 mb-5">
                                <a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                        
                                            <label class="home-upcommingtrek-card-label primary-color">Vikram</label>
                                    </div>
                                            </a>
                                            <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Farout Himalayas</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">100 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. </p>
                                        </div>
                                </div>
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a href="#">
                                <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                      
                                            <label class="home-upcommingtrek-card-label primary-color">Alexa</label>
                                </div>
                                            </a>
                                              <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">150 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Deserunt nemo culpa at asperiores corporis</p>
                                        </div>
                                    </div>
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                      <label
                                                class="home-upcommingtrek-card-label primary-color">Bella</label>
                                    </div>
                                                </a>
                                                  <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">200 Treks Completed</h5>
                                            <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eum, a. Exercitationem, tempora.</p>
                                        </div>
                                    </div>
                            
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                       <label
                                                class="home-upcommingtrek-card-label primary-color">Emma</label>
                                    </div>
                                                </a>
                                                 <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">100 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor ab, officiis odit vero ut fugit.</p>
                                        </div>
                                    </div>
                                </div>
                        
                         <div class="row">
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                       <label
                                                class="home-upcommingtrek-card-label primary-color">Amanda</label>
                                    </div>
                                                </a>
                                                 <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">80 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. </p>
                                        </div>
                                    </div>
                              
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a href="#">
                                <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                       <label
                                                class="home-upcommingtrek-card-label primary-color">Bella</label>
                                </div>
                                                </a>
                                                 <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">50 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Deserunt nemo culpa at asperiores corporis</p>
                                        </div>
                                    </div>
                                
                            <div class="col-12 col-md-4 col-lg-3 mb-5">
                                <a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                title="Lorem Ipsum"
                                                class="home-trekleader-image" />
                                       <label
                                                class="home-upcommingtrek-card-label primary-color">Amanda</label>
                                    </div>
                                                </a>
                                                 <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">100 Treks Completed</h5>
                                            <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eum, a. Exercitationem, tempora.</p>
                                        </div>
                                    </div>
                               
                            <div class="col-12 col-md-4 col-lg-3 mb-5"><a
                                    href="#">
                                    <div class="home-trekleader-card">
                                            <img src="~/assets/uploads/person.jpg"
                                                alt="Lorem Ipsum"
                                                class="home-trekleader-image">
                                       <label
                                                class="home-upcommingtrek-card-label primary-color">Emma</label>
                                    </div>
                                                </a>
                                                 <div class="home-upcommingtrek-card-description">
                                                <p class="trek-company">Lorem Ipsum</p>
                                                <div class="stars-box text-left">
                                                    <div class="stars-outer">
                                                        <div class="stars-inner">
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star checked" aria-hidden="true"></i>
                                                            <i class="fa fa-star" aria-hidden="true"></i>
                                                        </div>
                                                    </div>
                                                    <p>4.6 Rating</p>
                                                </div>
                                                <h5 class="number-treks">100 Treks Completed</h5>
                                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor ab, officiis odit vero ut fugit.</p>
                                        </div>
                                    </div>
                               </div>
                        </div> 
</div>
        


        <div class="home-newsletter">
    <div class="container">
        <div class="newsletter-box">
            <h2 class="altra-row-title text-center">Subscribe to stay connected</h2>
            <p class="altra-row-text text-center">Sign up and we’ll send you the details of upcoming treks</p>
            <form class="newsletter_frm">
        <input type="email" id="newsletter" placeholder="Enter your mail">
        <input type="submit" class="button-color newsletter_btn" value="Subscribe">
    </form>
    </div>
    </div>
</div>

<div class="footer">
    <div class="container">
      <div class="row">
        <div class="col-12 col-md-4 col-lg-3">
            <div class="widget">
            <h2>Useful Links</h2>
            <ul>
                <li><a href="#">Lorem</a></li>
                <li><a href="#">Lorem</a></li>
                <li><a href="#">Lorem</a></li>
                <li><a href="#">Lorem</a></li>
                <li><a href="#">Lorem</a></li>
            </ul>
        </div>
        </div>  
        <div class="col-12 col-md-4 col-lg-3">
            <div class="widget">
                <h2>Service Links</h2>
                <ul>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                </ul>
            </div>
        </div>  
        <div class="col-12 col-md-4 col-lg-3 ">
            <div class="widget">
                <h2>Other Links</h2>
                <ul>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                </ul>
            </div>
        </div>  
        <div class="col-12 col-md-4 col-lg-3">
            <div class="widget">
                <h2>Useful Links</h2>
                <ul>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                    <li><a href="#">Lorem</a></li>
                </ul>
            </div>
        </div>  
      </div> <!-- row ends -->
        <div class="col-12 col-md-12 col-lg-12">
            <hr />
            <p class="copyright">2020-2021 Explore alternate travels Pvt Ltd. All Rights Reserved.</p>
        </div>
    </div>
</div>

        
                        
                       
                            </div>
                        </div>
                    </div>
     
</template>
<script>
export default {};
</script>